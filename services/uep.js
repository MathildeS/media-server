...
function getContent(pendingRequests) {
	...
	var channel='' ;
	if (GLOBAL_config.CALL_TYPE === 'RBB') channel='AND domain_source:rbb';
	else if (GLOBAL_config.CALL_TYPE === 'SV') channel+='AND domain_source:sv'; 
    if (GLOBAL_config.DEBUG) console.log(currentService + ' *** ' + query);
    var options = {
          url: 'http://ir.lmcloud.vse.cz/irapi/media-server/?q=(media_title:*' +
              encodeURIComponent(query) +'* OR media_description:*' + encodeURIComponent(query)  + '*) ' + channel,
